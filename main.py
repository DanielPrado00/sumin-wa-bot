...
def extract_items_for_quote(text: str, history: list) -> tuple[list[dict], str]:
    ...
    prompt = (
        ...
        "  • 'ROLLO DE MICROALAMBRE MIG 0.035 X 33LB RF56-ER70S-6'\n"
        "     → product='microalambre ER70S-6 0.035 33 LBS' quantity=1 unit='ROLLO'\n"
        "     (el '33LB' es el peso del rollo — la presentación; NO la cantidad.)\n"
        "  • '2 rollos de microalambre 0.045 de 11 lbs'\n"
        "     → product='microalambre 0.045 11 LBS' quantity=2 unit='ROLLO'\n"
        "  • '5 cajas de electrodo 6011 1/8 de 50 lbs c/u'\n"
        "     → product='electrodo 6011 1/8 50 LBS' quantity=5 unit='CAJA'\n"
        "Patrón general: si hay un peso pegado al producto (con 'X', 'de', 'por'\n"
        "o sin separador como '33LB'), MANTÉN el peso EN el campo product, en\n"
        "formato 'N LBS' (con espacio). Solo es cantidad+unidad cuando el\n"
        "cliente claramente pide 'quiero N libras DE algo' (sin que el peso\n"
        "describa el empaque). Si dudás, dejá el peso en product.\n"
        "\n"
        "⚠️ NO RESUMAS el producto: si el cliente da specs como '0.035', '33 LB',\n"
        "'RF56', 'ER70S-6', 'LINCOLN', preserválas TODAS en el campo product —\n"
        "el matcher las usa para discriminar variants del catálogo.\n\n"
